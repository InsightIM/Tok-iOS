...
@implementation FCOggOpusReader {
    NSString *_path;
    OggOpusFile *_file;
...
+ (nullable instancetype)readerWithFileAtPath:(NSString *)path error:(NSError * _Nullable *)outError {
    return [[FCOggOpusReader alloc] initWithFileAtPath:path error:outError];
}
...
- (NSData * _Nullable)pcmDataWithMaxLength:(NSUInteger)maxLength
    ...
    if (result < 0) {
        if (outError) {
            *outError = ErrorWithCodeAndOpusErrorCode(FCOggOpusErrorCodeRead, result);
        }
        return nil;
    } else {
        bytesRead = result * 2;
