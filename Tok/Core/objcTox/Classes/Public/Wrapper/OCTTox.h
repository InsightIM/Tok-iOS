...
@property (strong, nonatomic) NSArray *dhtArray;
...
//- (NSString *)publicKeyFromPeerNumber:(OCTToxFriendNumber)peerNumber conferenceNumber:(uint32_t)conferenceNumber error:(NSError **)error;
 ...
 *
 * The message id. Message IDs are unique per friend. The first message ID is 0. Message IDs are
...
/**
 * Send an offline message to bot.
 */
...
/**
 * Send a stranger message to bot.
 */
