...
- (instancetype)initWithOptions:(OCTToxOptions *)options savedData:(NSData *)data error:(NSError **)error
{
    ...
    tox_options_set_ipv6_enabled(options.options, false);
...
- (void)directUDPConnection
{
    if (self.isConnected) {
        return;
    }
    self.isConnected = YES;
    for (NSDictionary *dhtItem in self.dhtArray) {
        OCTToxFriendNumber friendNumber = [dhtItem[@"friendNumber"] intValue];
        NSString *dhtPublicKey = dhtItem[@"dhtPublicKey"];
        NSString *host = dhtItem[@"host"];
        NSNumber *port = dhtItem[@"port"];
        
        [self addDHTByIPWithFriendNumber:friendNumber publicKey:dhtPublicKey host:host port:port.unsignedIntValue];
    }
}
...
- (BOOL)addDHTTCPRelayWithFriendNumber:(OCTToxFriendNumber)friendNumber dhtPublicKey:(NSString *)dhtPublicKey host:(NSString *)host port:(OCTToxPort)port publicKey:(NSString *)publicKey error:(NSError **)error
{
    NSParameterAssert(host);
    NSParameterAssert(publicKey);
    ...
    uint8_t *cDHTPublicKey = [OCTTox hexStringToBin:dhtPublicKey];
    
    TOX_ERR_FRIEND_SET_DHT_NODE cError;
    bool result = tox_frined_set_dht_tcp_relay_node(self.tox, friendNumber, cDHTPublicKey, cPublicKey, (const uint8_t *)cAddress, port, &cError);
    
    if (cError != TOX_ERR_FRIEND_SET_DHT_NODE_OK) {
        [self fillError:error withCErrorBootstrap:TOX_ERR_BOOTSTRAP_NULL];
    }
...
- (BOOL)addDHTByIPWithFriendNumber:(OCTToxFriendNumber)friendNumber publicKey:(NSString *)publicKey host:(NSString *)host port:(OCTToxPort)port
{
    const uint8_t *cHost = (const uint8_t *)[host cStringUsingEncoding:NSUTF8StringEncoding];
    const uint8_t *cPublicKey = [OCTTox hexStringToBin:publicKey];
    BOOL result = tox_frined_set_dht_ip_port(self.tox, friendNumber, cPublicKey, cHost, port, 2000, (__bridge void *)(self));
    return result;
}
                                         ...
                                         error:(NSError **)error
{
    ...
    switch (type) {
        ...
        case OCTToxMessageTypeStranger:
            return -1;
    }
    ...
    tox_friend_send_message(self.tox, friendNumber, cType, cMessage, message.length, messageId, &cError);
                                                   ...
                                                   error:(NSError **)error
{
    ...
    switch (offlineCmd) {
        ...
        case OCTToxMessageOfflineCmdPushToken:
            ...
            break;
        case OCTToxMessageOfflineCmdFilePull:
            cmd = TOX_MESSAGE_OFFLINE_FILE_PULL_REQUEST;
            break;
        case OCTToxMessageOfflineCmdFileCancel:
            cmd = TOX_MESSAGE_OFFLINE_FILE_CANCEL_REQUEST;
            break;
        case OCTToxMessageOfflineCmdVersionInfoRequest:
            cmd = TOX_MESSAGE_OFFLINE_VERSION_INFO_REQUEST;
            break;
        case OCTToxMessageOfflineCmdVersionInfoResponse:
            cmd = TOX_MESSAGE_OFFLINE_VERSION_INFO_RESPONSE;
            break;
    }
    ...
    uint8_t cMessage[message.length];
    [message getBytes:&cMessage length:message.length];
    tox_friend_send_message_offline(self.tox, botFriendNumber, cmd, (const uint8_t *)cMessage, message.length, messageId, &cError);
...
- (OCTToxMessageId)sendGroupMessageWithBotFriendNumber:(OCTToxFriendNumber)botFriendNumber
                                                 ...
                                                 error:(NSError **)error
{
    ...
    TOX_MESSAGE_GROUP_CMD cmd = (TOX_MESSAGE_GROUP_CMD)groupCmd;
    
    TOX_ERR_FRIEND_SEND_MESSAGE cError;
    uint8_t cMessage[message.length];
    [message getBytes:&cMessage length:message.length];
    tox_group_send_message(self.tox, botFriendNumber, cmd,  (const uint8_t *)cMessage, message.length, messageId, &cError);
...
- (OCTToxMessageId)sendStrangerMessageWithBotFriendNumber:(OCTToxFriendNumber)botFriendNumber
                                                    ...
                                                    error:(NSError **)error
{
    ...
    TOX_MESSAGE_STRANGER_CMD cmd = (TOX_MESSAGE_STRANGER_CMD)strangerCmd;
    
    TOX_ERR_FRIEND_SEND_MESSAGE cError;
    uint8_t cMessage[message.length];
    [message getBytes:&cMessage length:message.length];
    tox_stranger_send_message(self.tox, botFriendNumber, cmd, (const uint8_t *)cMessage, message.length, messageId, &cError);
                               ...
                               error:(NSError **)error
{
    ...
    bool result;
    result = tox_file_control(self.tox, friendNumber, fileNumber, cControl, &cError);
                        ...
                        error:(NSError **)error
{
    ...
    bool result;
    result = tox_file_seek(self.tox, friendNumber, fileNumber, position, &cError);
                                       ...
                                       error:(NSError **)error
{
    ...
    switch (kind) {
        ...
        case OCTToxFileKindAvatar:
            ...
            break;
        case OCTToxFileKindNodesFile:
            cKind = TOX_FILE_KIND_NODES_FILE;
            break;
    }
    ...
    OCTToxFileNumber result;
    result = tox_file_send(self.tox, friendNumber, cKind, fileSize, cFileId, cFileName, fileName.length, &cError);
...
+ (OCTToxMessageId)uniqueMessageId
{
    return tox_local_msg_id();
}
...
- (NSData *)encryptOfflineMessage:(OCTToxFriendNumber)friendNumber message:(NSString *)message
{
    ...
    int len = tox_encrypt_offline_message(self.tox, friendNumber, (const uint8_t *)cMessage, length, encryptMessage, nil);
...
- (NSData *)decryptOfflineMessage:(NSData *)message friendNumber:(OCTToxFriendNumber)friendNumber
{
    ...
    NSUInteger len = tox_decrypt_offline_message(self.tox, friendNumber, cMessage, message.length, decryptMessage, nil);
...
+ (BOOL)checkSocks5WithHost:(NSString *)host port:(NSUInteger)port
{
    const uint8_t *cHost = (const uint8_t *)[host cStringUsingEncoding:NSUTF8StringEncoding];
    return tox_is_socket5(cHost, (uint32_t)port, 5) == 0;
}

+ (BOOL)checkBootstrapNode:(NSString *)host port:(NSUInteger)port isTCP:(BOOL)isTCP publicKey:(NSString *)publicKey
{
    const uint8_t *cHost = (const uint8_t *)[host cStringUsingEncoding:NSUTF8StringEncoding];
    const uint8_t *cPublicKey = (const uint8_t *)[publicKey cStringUsingEncoding:NSUTF8StringEncoding];
    return tox_test_node(cHost, (uint16_t)port, isTCP, cPublicKey, 3) == 0;
}

+ (NSString *)dynamicDomainWith:(NSTimeInterval)dateInterval
{
    int size = 100;
    uint8_t *cBuffer = malloc(size);
    uint64_t mkTime = dateInterval;
    const char *cUrl = tox_get_url((char *)cBuffer, size, mkTime);
    NSString *url = [[NSString alloc] initWithCString:cUrl encoding:NSUTF8StringEncoding];
    return url;
}
...
- (void)setupCallbacks
{
    ...
    tox_callback_stranger_message(_tox, strangerMessageCallback);
}
...
- (OCTToxMessageType)messageTypeFromCMessageType:(TOX_MESSAGE_TYPE)cType
{
    switch (cType) {
        ...
        case TOX_MESSAGE_TYPE_STRANGER:
            return OCTToxMessageTypeStranger;
    }
}
...
//    NSString *description = @"Cannot add friend";
    ...
    *error = [OCTTox createErrorWithCode:code description:failureReason failureReason:failureReason];
...
- (BOOL)shareIdIsValid:(NSString *)shareId
{
    const char *cShareId = [shareId cStringUsingEncoding:NSUTF8StringEncoding];
    return tox_share_id_is_invalid((const uint8_t *)cShareId);
}

+ (NSString *)binToHexString:(NSData *)data
{
    return [OCTTox binToHexString:(uint8_t *)data.bytes length:data.length];
}
...
void connectionStatusCallback(Tox *cTox, TOX_CONNECTION cStatus, void *userData)
{
    ...
    dispatch_async(tox.workQueue, ^{
        NSLog(@"%@ connectionStatusCallback with status %lu", tox, (unsigned long)status);
...
void friendConnectionStatusCallback(Tox *cTox, uint32_t friendNumber, TOX_CONNECTION cStatus, void *userData)
{
    ...
    NSLog(@"connectionStatusCallback with status %lu, friendNumber %d", (unsigned long)status, friendNumber);
...
void friendReadReceiptCallback(Tox *cTox, uint32_t friendNumber, uint32_t messageId, int64_t local_msg_id, void *userData)
{
    OCTTox *tox = (__bridge OCTTox *)(userData);
    
    OCTLogCInfo(@"readReceiptCallback with local message id %lld, friendNumber %d", local_msg_id, friendNumber);
    
    dispatch_async(tox.workQueue, ^{
        if ([tox.delegate respondsToSelector:@selector(tox:messageDelivered:friendNumber:)]) {
            [tox.delegate tox:tox messageDelivered:local_msg_id friendNumber:friendNumber];
        }
    });
}
                         ...
                         void *userData)
{
    ...
    switch (cKind) {
        ...
        case TOX_FILE_KIND_AVATAR:
            ...
            break;
        case TOX_FILE_KIND_NODES_FILE:
            kind = OCTToxFileKindNodesFile;
            break;
    }
                            ...
                            void *userData)
{
    ...
    switch (cCmd) {
        ...
        case TOX_MESSAGE_DEVICE_REQUEST:
            ...
            break;
        case TOX_MESSAGE_OFFLINE_FILE_PULL_REQUEST:
            cmd = OCTToxMessageOfflineCmdFilePull;
            break;
        case TOX_MESSAGE_OFFLINE_FILE_CANCEL_REQUEST:
            cmd = OCTToxMessageOfflineCmdFileCancel;
            break;
        case TOX_MESSAGE_OFFLINE_VERSION_INFO_REQUEST:
            cmd = OCTToxMessageOfflineCmdVersionInfoRequest;
            break;
        case TOX_MESSAGE_OFFLINE_VERSION_INFO_RESPONSE:
            cmd = OCTToxMessageOfflineCmdVersionInfoResponse;
            break;
    }
...
void groupMessageCallback(Tox *cTox,
                          uint32_t friendNumber,
                          TOX_MESSAGE_GROUP_CMD cCmd,
                          const uint8_t *cMessage,
                          size_t length,
                          uint8_t device_type,
                          uint32_t version_code,
                          void *userData)
{
    ...
    OCTToxGroupCmd cmd = (OCTToxGroupCmd)cCmd;
    NSData *message = [NSData dataWithBytes:cMessage length:length];
    
    dispatch_async(tox.workQueue, ^{
        if ([tox.delegate respondsToSelector:@selector(tox:friendNumber:groupCmd:messageData:length:)]) {
            [tox.delegate tox:tox friendNumber:friendNumber groupCmd:cmd messageData:message length:length];
        }
    });
}

void strangerMessageCallback(Tox *cTox,
                             uint32_t friendNumber,
                             TOX_MESSAGE_STRANGER_CMD cCmd,
                             const uint8_t *cMessage,
                             size_t length,
                             uint8_t device_type,
                             uint32_t version_code,
                             void *userData)
{
    ...
    OCTToxStrangerCmd cmd = (OCTToxStrangerCmd)cCmd;
    NSData *message = [NSData dataWithBytes:cMessage length:length];
    
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([tox.delegate respondsToSelector:@selector(tox:friendNumber:strangerCmd:messageData:length:)]) {
            [tox.delegate tox:tox friendNumber:friendNumber strangerCmd:cmd messageData:message length:length];
        }
    });
}
