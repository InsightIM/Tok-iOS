...
- (uint8_t)deviceType
{
    return tox_options_get_device_type(self.options);
}

- (void)setDeviceType:(uint8_t)deviceType
{
    tox_options_set_device_type(self.options, deviceType);
}

- (uint32_t)versionCode
{
    return tox_options_get_version_code(self.options);
}

- (void)setVersionCode:(uint32_t)versionCode
{
    tox_options_set_version_code(self.options, versionCode);
}
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    options.deviceType = self.deviceType;
    options.versionCode = self.versionCode;
