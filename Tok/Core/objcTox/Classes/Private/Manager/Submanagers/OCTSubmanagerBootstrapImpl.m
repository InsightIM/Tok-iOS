...
@property (strong, nonatomic) NSMutableArray<OCTNode *> *addedNodes;

@property (strong, nonatomic) NSArray<NSDictionary *> *dhtArray;
...
- (instancetype)init
{
    ...
    _addedNodes = [NSMutableArray new];
...
- (void)bootstrapWithDHTRelayArray:(NSArray<NSDictionary *> *)dhtArray
{
    _dhtArray = dhtArray;
...
- (void)tryToBootstrap
{
    if ([self.dataSource managerIsToxConnected]) {
        NSLog(@"trying to bootstrap... tox is connected, exiting");
    ...
    if (! selectedNodes.count) {
        OCTLogDebug(@"trying to bootstrap... no nodes left, exiting");
        [self finishBootstrapping];
        return;
    }
    
    OCTLogDebug(@"trying to bootstrap... picked %lu nodes", (unsigned long)selectedNodes.count);
...
- (void)safeBootstrapFromHost:(NSString *)host port:(OCTToxPort)port publicKey:(NSString *)publicKey
{
    ...
    if (! [[self.dataSource managerGetTox] bootstrapFromHost:host port:port publicKey:publicKey error:&error]) {
        OCTLogDebug(@"trying to bootstrap... bootstrap failed with address %@, error %@", host, error);
    }
}
...
{
    NSMutableArray<OCTNode *> *allNodes;
    NSMutableArray *selectedNodes = [NSMutableArray new];

    @synchronized(self.addedNodes) {
        allNodes = [self.addedNodes mutableCopy];
    }
    ...
    @synchronized(self.addedNodes) {
        [self.addedNodes removeObjectsInArray:selectedNodes];
    }
