...
private struct Constants {
    static let ActiveAccountDataService = "com.insight.KeychainManager.ActiveAccountDataService"

    static let toxPasswordForActiveAccount = "PasswordForActiveAccount"
...
private extension KeychainManager {
    ...
    func getDataForKey(_ key: String) -> Data? {
        ...
        guard status == noErr else {
            print("Error when getting keychain data for key \(key), status \(status)")
            return nil
        }

        guard let data = queryResult as? Data else {
            print("Unexpected data for key \(key)")
            return nil
        }
    ...
    func setData(_ newData: Data?, forKey key: String) {
        ...
        switch (oldData, newData) {
            case (.some(_), .some(let data)):
                ...
                guard status == noErr else {
                    print("Error when updating keychain data for key \(key), status \(status)")
                    return
                }
            ...
            case (.none, .some(let data)):
                ...
                guard status == noErr else {
                    print("Error when setting keychain data for key \(key), status \(status)")
                    return
                }
