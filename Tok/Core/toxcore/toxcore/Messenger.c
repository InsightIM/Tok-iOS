...
//int32_t get_bot_number(Messenger *m)
//{
//    char bot_pk_hex[] = "474406DEFBB732BC8BB4CCFC0EE8F68545D7001E1482BF2012427A2F1E604175";
//    VLA(uint8_t, bot_pk, CRYPTO_PUBLIC_KEY_SIZE);
//    sodium_hex2bin(bot_pk, sizeof(bot_pk), bot_pk_hex, sizeof(bot_pk_hex), NULL, NULL, NULL);
//    int32_t botnumber = getfriend_id(m, bot_pk);
//    return botnumber;
//}

int32_t get_group_bot_number(Messenger *m)
{
    char bot_pk_hex[] = "FDE9226FCB835B584E9854EA0313ECA663DBAB14863A93339B0C7555975D0148";
...
int m_upload_push_token(Messenger *m, const uint8_t *token, uint32_t length)
{
    int32_t botnumber = get_group_bot_number(m);
    
    VLA(uint8_t, packet, 1 + 1 + length);
    // 1字节的CMD
    packet[0] = GROUP_CMD_PUSH_TOKEN;
    // 1字节的平台
    packet[1] = 0;
    // 放入消息
    memcpy(packet + 1 + 1, token, length);

    uint32_t message_id = 0;
    return m_send_message_generic(m, botnumber, MESSAGE_GROUP, packet, SIZEOF_VLA(packet), &message_id);
}

int m_create_group(Messenger *m)
{
    int32_t botnumber = get_group_bot_number(m);
    VLA(uint8_t, packet, 1);
    packet[0] = GROUP_CMD_CREAET;
    uint32_t message_id = 0;
    return m_send_message_generic(m, botnumber, MESSAGE_GROUP, packet, 1, &message_id);
}

int m_invite_group(Messenger *m, int32_t friendnumber, int32_t groupnumber)
{
    int32_t botnumber = get_group_bot_number(m);
    
    VLA(uint8_t, packet, 1 + sizeof(int32_t) + CRYPTO_PUBLIC_KEY_SIZE);
    packet[0] = GROUP_CMD_INVITE;
    memcpy(packet + 1, &groupnumber, sizeof(int32_t));
    
    uint8_t *pk = m->friendlist[friendnumber].real_pk;
    memcpy(packet + 1 + sizeof(int32_t), pk, CRYPTO_PUBLIC_KEY_SIZE);

    uint32_t message_id = 0;
    return m_send_message_generic(m, botnumber, MESSAGE_GROUP, packet, SIZEOF_VLA(packet), &message_id);
}

int m_send_group_message(Messenger *m, int32_t groupnumber,  const uint8_t *message, uint32_t length)
{
    int32_t botnumber = get_group_bot_number(m);
    
    VLA(uint8_t, packet, 1 + SIZEOF_VLA(int32_t) + length);
    // 1字节的CMD
    packet[0] = GROUP_CMD_MESSAGE;
    // int32_t groupnumber
    memcpy(packet + 1, &groupnumber, SIZEOF_VLA(int32_t));
    // 放入消息
    memcpy(packet + 1 + SIZEOF_VLA(int32_t), message, length);
    uint32_t message_id = 0;
    return m_send_message_generic(m, botnumber, MESSAGE_GROUP, packet, SIZEOF_VLA(packet), &message_id);
}

int m_fetch_group_peer_list(Messenger *m, int32_t groupnumber)
{
    int32_t botnumber = get_group_bot_number(m);
    VLA(uint8_t, packet, 1 + SIZEOF_VLA(int32_t));
    // 1字节的CMD
    packet[0] = GROUP_CMD_GET_PEERLIST;
    // int32_t groupnumber
    memcpy(packet + 1, &groupnumber, SIZEOF_VLA(int32_t));
...
int m_fetch_peer_name(Messenger *m, int32_t groupnumber, uint32_t peernumber)
{
    int32_t botnumber = get_group_bot_number(m);
    VLA(uint8_t, packet, 1 + SIZEOF_VLA(int32_t) + SIZEOF_VLA(int32_t));
    // 1字节的CMD
    packet[0] = GROUP_CMD_GET_PEER_NAME;
    // int32_t groupnumber
    memcpy(packet + 1, &groupnumber, SIZEOF_VLA(int32_t));
    memcpy(packet + 1 + SIZEOF_VLA(int32_t), &peernumber, SIZEOF_VLA(int32_t));

    uint32_t message_id = 0;
    return m_send_message_generic(m, botnumber, MESSAGE_GROUP, packet, SIZEOF_VLA(packet), &message_id);
}

int m_fetch_peer_pk(Messenger *m, int32_t groupnumber, uint32_t peernumber)
{
    int32_t botnumber = get_group_bot_number(m);
    VLA(uint8_t, packet, 1 + SIZEOF_VLA(int32_t) + SIZEOF_VLA(int32_t));
    // 1字节的CMD
    packet[0] = GROUP_CMD_GET_PEER_PUBLICKEY;
    // int32_t groupnumber
    memcpy(packet + 1, &groupnumber, SIZEOF_VLA(int32_t));
    memcpy(packet + 1 + SIZEOF_VLA(int32_t), &peernumber, SIZEOF_VLA(int32_t));
...
int m_set_group_title(Messenger *m, int32_t groupnumber, const uint8_t *title)
{
    int32_t botnumber = get_group_bot_number(m);
    VLA(uint8_t, packet, 1 + SIZEOF_VLA(int32_t));
    // 1字节的CMD
    packet[0] = GROUP_CMD_SET_TITLE;
    // int32_t groupnumber
    memcpy(packet + 1, &groupnumber, SIZEOF_VLA(int32_t));
 ...
 * return 0 if success.
 */
int m_send_message_group_generic(Messenger *m, int32_t friendnumber, uint8_t type, const uint8_t *message, uint32_t length,
                                 uint32_t message_id)
{
    ...
    m->friendlist[friendnumber].message_id = message_id;
...
{
    m->assist_message = function;
}

/* Set the function that will be executed when a message from a friend is received. */
void m_callback_normalmessage(Messenger *m, m_normal_message_cb *function)
{
    m->normal_message = function;
}

/* Set the function that will be executed when a echo message from a someone is received. */
void m_callback_echomessage(Messenger *m, m_echo_message_cb *function)
{
    m->echo_message = function;
}

/* Set the function that will be executed when a confirm message from a someone is received. */
void m_callback_confirmmessage(Messenger *m, m_confirm_message_cb *function)
{
...
static int m_handle_packet(void *object, int i, const uint8_t *temp, uint16_t len, void *userdata)
{
    ...
    switch (packet_id) {
        ...
        case PACKET_ID_MESSAGE: // fall-through
		{
            ...
            uint32_t time = 0;
			uint32_t magic = 0;
			memcpy(&magic, message, sizeof(uint32_t));
			if (0X6C1 == magic) {
				if (m->normal_message) {
					(*m->normal_message)(m, i, type, message_terminated, message_length, time, userdata);
				} 
			}else {
				if (m->friend_message) {
					(*m->friend_message)(m, i, type, message_terminated, message_length, time, userdata);
				}
			}
            break;
		}
        case PACKET_ID_ACTION:
        case PACKET_ID_BOT_MESSAGE:
        case PACKET_ID_GROUP_MESSAGE: {
            if (data_length == 0) {
                break;
            }
            ...
            //online messge time is 0
            //todo group offline message shoud take send time
            uint32_t time = 0;
		...
		case PACKET_ID_ASSIST: {
            ...
            uint32_t time = 0;
            if (m->assist_message) {
                (*m->assist_message)(m, i, type, message_terminated, message_length, time, userdata);
            }
            break;
		}
        case PACKET_ID_FORWARD_MESSAGE: {
            if (data_length == 0) {
                break;
            }
            ...
            if (m->friend_message) {
                ...
                // messge format: timestamp+pk+nonce+encrypt_data


                uint64_t time_len = sizeof(uint64_t);
                uint64_t time = 0;
                memcpy(&time, message, time_len);

                VLA(uint8_t, friendpk, CRYPTO_PUBLIC_KEY_SIZE);
                memcpy(friendpk, message+time_len, CRYPTO_PUBLIC_KEY_SIZE);
                uint32_t friendnumber = getfriend_id(m, friendpk);
                ...
                VLA(uint8_t, encrypted_message, message_length - CRYPTO_PUBLIC_KEY_SIZE - CRYPTO_NONCE_SIZE-time_len);
                memcpy(encrypted_message, message + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_NONCE_SIZE+time_len, message_length - CRYPTO_PUBLIC_KEY_SIZE - CRYPTO_NONCE_SIZE-time_len);
                
                uint8_t real_message[message_length - CRYPTO_PUBLIC_KEY_SIZE - CRYPTO_NONCE_SIZE - CRYPTO_MAC_SIZE-time_len];
                //VLA(uint8_t, encrypted, message_length - CRYPTO_PUBLIC_KEY_SIZE - CRYPTO_NONCE_SIZE - time_len);
                
                const uint8_t *secret_key = nc_get_self_secret_key(m->net_crypto);
                int32_t len = decrypt_data(friendpk, secret_key, nonce, encrypted_message,  message_length - CRYPTO_PUBLIC_KEY_SIZE - CRYPTO_NONCE_SIZE-time_len, real_message);
                
                (*m->friend_message)(m, friendnumber, type, real_message, len, time, userdata);
            }
            
            break;
        }
		case PACKET_ID_MESSAGE_ECHO: {
            if (data_length == 0) {
                break;
            }
            const uint8_t *message = data;
            ...
            uint32_t time = 0;
            if (m->echo_message) {
                (*m->echo_message)(m, i, type, message_terminated, message_length, time, userdata);
            }
            break;
		}
		case PACKET_ID_MESSAGE_CONFIRM: {
            if (data_length == 0) {
                break;
            }
            const uint8_t *message = data;
            ...
            //online messge time is 0
            uint32_t time = 0;
            if (m->confirm_message) {
                (*m->confirm_message)(m, i, type, message_terminated, message_length, time, userdata);
            }
            break;
		}
...
int m_encrypt_offline_message(Messenger *m, const uint32_t friendnumber, const uint8_t *message, const int length , uint8_t *encrypt_message){
    ...
    //sodium_bin2hex(hex, total_length*2+1, encrypted_message, , sizeof(to_pk));
