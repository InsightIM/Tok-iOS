...
typedef enum TOX_MESSAGE_TYPE {
    ...
    /**
     * A message describing send to bot action.
     */
    TOX_MESSAGE_TYPE_BOT,

    /**
     * A message describing a bot forward action.
     */
    TOX_MESSAGE_TYPE_FORWARD,

    /**
     * A message describing a group action.
     */
	...
	/**
	 * A message describing a confirm action.
	 */
	TOX_MESSAGE_TYPE_CONFIRM = 37,
	/**
	 * A message describing for assist, such as give a version code
	 */
	TOX_MESSAGE_TYPE_ASSIST = 38,
	/**
	 * message end
	 */
...
#define MAGIC_NUMBER  0X6C1
/**
 * messages head, 12 bytes
 * @param magic use low 16 bit, high 16 bit reverve
 * @param version such as 100000, that means 1.0.0
 */
struct Tox_Msg_Head{
	uint32_t magic;
	uint16_t cmd;
...
/*
 * Message define for single messages.
 * To make sure no message lost, we add response/confirm for most of request, 
 * and waiting the message arrive in a few seconds, if not get, resend it.
 * Sometime it maybe a problem, the request get but response lost, so we resend, a peer will
 * get more than one message, we filter out duplicate messages by a local id which the sender
 * generated it.
*/
typedef enum TOX_FRIEND_CMDS {
    TOX_FRIEND_CMD_MSG_REQ,
    TOX_FRIEND_CMD_MSG_RES,
    TOX_FRIEND_CMD_MSG_CFM,
    TOX_FRIEND_CMD_MSG_STORE_REQ,
    TOX_FRIEND_CMD_MSG_STORE_RES,
    TOX_FRIEND_CMD_MSG_STORE_CFM,
    TOX_FRIEND_CMD_MSG_TO_READ_REQ,
    TOX_FRIEND_CMD_MSG_TO_READ_RES,
    TOX_FRIEND_CMD_MSG_PULL_REQ,
    TOX_FRIEND_CMD_MSG_PULL_RES,
    TOX_FRIEND_CMD_MSG_DEL_REQ
} TOX_FRIEND_CMDS;


/*
 * message define for group message. 
*/
typedef enum TOX_GROUP_CMDS {
    TOX_GROUP_CMD_CREAET,
    TOX_GROUP_CMD_INVITE,
    TOX_GROUP_CMD_MESSAGE,
    TOX_GROUP_CMD_GET_PEERLIST,
    TOX_GROUP_CMD_GET_PEER_NAME,
    TOX_GROUP_CMD_GET_PEER_PUBLICKEY,
    TOX_GROUP_CMD_GET_TITLE,
    TOX_GROUP_CMD_SET_TITLE,
    TOX_GROUP_CMD_LEAVE,
    TOX_PUSH_TOKEN,
    TOX_GROUP_CMD_CREAET_CB = 21,
    TOX_GROUP_CMD_BE_INVITEED,
    TOX_GROUP_CMD_RECEIVED_MSG,
    TOX_GROUP_CMD_PEERLIST_CHANGED,
    TOX_GROUP_CMD_PEER_NAME_CHANGED,
    TOX_GROUP_CMD_PEER_PUBLICKEY_CB,
    TOX_GROUP_CMD_TITLE_CHANGED,
    TOX_GROUP_CMD_KICKEDOUT,
    TOX_GROUP_CMD_MSG_STORE_REQ,
    TOX_GROUP_CMD_MSG_STORE_RES,
    TOX_GROUP_CMD_MSG_STORE_CFM = 31,
	TOX_GROUP_CMD_MSG_TO_READ_REQ,
	TOX_GROUP_CMD_MSG_TO_READ_RES,
    TOX_GROUP_CMD_MSG_DEL_REQ,
    TOX_GROUP_CMD_MSG_PULL_REQ,
...
uint32_t tox_friend_send_message_store_req(Tox *tox, uint32_t friend_number, const uint8_t *message,
                                 size_t length, TOX_ERR_FRIEND_SEND_MESSAGE *error);
/**
 * send group message store request,
 * @param friend_number The friend number of the friend to send the message to.
 * @param message A non-NULL pointer to the first element of a byte array
 *   containing the message text.
 * @param length Length of the message to be sent.
...
uint32_t tox_friend_send_message_del_req(Tox *tox, uint32_t friend_number, const uint8_t *message,
                                 size_t length, TOX_ERR_FRIEND_SEND_MESSAGE *error);
/**
 * send group message del request,
 * @param friend_number The friend number of the friend to send the message to.
 * @param message A non-NULL pointer to the first element of a byte array
 *   containing the message text.
 * @param length Length of the message to be sent.
...
/**
 * send group message to read  response,
 * @param friend_number The friend number of the friend to send the message to.
 * @param message A non-NULL pointer to the first element of a byte array
 *   containing the message text.
 * @param length Length of the message to be sent.
...
uint32_t tox_friend_send_message_store_cfm(Tox *tox, uint32_t friend_number, const uint8_t *message,
                                 size_t length, TOX_ERR_FRIEND_SEND_MESSAGE *error);
/**
 * send group message store confirm,
 * @param friend_number The friend number of the friend to send the message to.
 * @param message A non-NULL pointer to the first element of a byte array
 *   containing the message text.
 * @param length Length of the message to be sent.
 ...
 */
typedef void tox_friend_message_store_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_friend_message_store_res_cb call function declare.
 */
void friend_message_store_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `friend_message_store_res` event. Pass NULL to unset.
 *
 * This event is triggered when a friend_message_store_res message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_friend_message_store_cfm_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_friend_message_store_cfm_cb call function declare.
 */
void friend_message_store_cfm_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `friend_message_store_cfm` event. Pass NULL to unset.
 *
 * This event is triggered when a friend_message_store_cfm message from a friend is received.
 */
 ...
 */
void friend_message_to_read_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `friend_message_to_read_req_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a friend_message_to_read_req message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_friend_message_to_read_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_friend_message_to_read_res_cb call function declare.
 */
void friend_message_to_read_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `friend_message_to_read_res_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a friend_message_to_read_res message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_friend_message_pull_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_friend_message_pull_res_cb call function declare.
 */
void friend_message_pull_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `friend_message_pull_res_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a friend_message_pull_res message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_group_message_store_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_group_message_store_res_cb call function declare.
 */
void group_message_store_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `group_message_store_res` event. Pass NULL to unset.
 *
 * This event is triggered when a group_message_store_res message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_group_message_store_cfm_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_group_message_store_cfm_cb call function declare.
 */
void group_message_store_cfm_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `group_message_store_cfm` event. Pass NULL to unset.
 *
 * This event is triggered when a group_message_store_cfm message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_group_message_to_read_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_group_message_to_read_req_cb call function declare.
 */
void group_message_to_read_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `group_message_to_read_req_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a group_message_to_read_req message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_group_message_to_read_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_group_message_to_read_res_cb call function declare.
 */
void group_message_to_read_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `group_message_to_read_res_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a group_message_to_read_res message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_group_message_pull_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_group_message_pull_res_cb call function declare.
 */
void group_message_pull_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `group_message_pull_res_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a group_message_pull_res message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_friend_message_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_friend_message_req_cb call function declare.
 */
void friend_message_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `friend_message_req_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a friend_message_req message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_friend_message_store_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_friend_message_store_req_cb call function declare.
 */
void friend_message_store_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `friend_message_store_req_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a friend_message_store_req message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_group_message_store_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_group_message_store_req_cb call function declare.
 */
void group_message_store_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `group_message_store_req_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a group_message_store_req message from a friend is received.
 */
void tox_callback_group_message_store_req(Tox *tox, tox_friend_message_store_req_cb *callback);
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_friend_message_pull_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_friend_message_pull_req_cb call function declare.
 */
void friend_message_pull_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `friend_message_pull_req_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a friend_message_pull_req message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_group_message_pull_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_group_message_pull_req_cb call function declare.
 */
void group_message_pull_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `group_message_pull_req_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a group_message_pull_req message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_friend_message_del_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_friend_message_del_req_cb call function declare.
 */
void friend_message_del_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `friend_message_del_req_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a friend_messagedel__req message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_group_message_del_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_group_message_del_req_cb call function declare.
 */
void group_message_del_req_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `group_message_del_req_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a group_messagedel__req message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_friend_message_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_friend_message_res_cb call function declare.
 */
void friend_message_res_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `friend_message_res_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a friend_message_res message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_friend_message_cfm_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
								   size_t length,  void *user_data);

/**
 * tox_friend_message_cfm_cb call function declare.
 */
void friend_message_cfm_cb(Tox *tox, uint32_t friend_number, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `friend_message_cfm_cb` event. Pass NULL to unset.
 *
 * This event is triggered when a friend_message_cfm message from a friend is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_friend_message_cb(Tox *tox, uint32_t friend_number, TOX_MESSAGE_TYPE type, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * tox_friend_message_cb call function declare.
 */
void friend_message_cb(Tox *tox, uint32_t friend_number, TOX_MESSAGE_TYPE type, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_normal_message_cb(Tox *tox, uint32_t friend_number, TOX_MESSAGE_TYPE type, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * tox_normal_message_cb call function declare.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_echo_message_cb(Tox *tox, uint32_t friend_number, TOX_MESSAGE_TYPE type, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * tox_echo_message_cb call function declare.
 */
void echo_message_cb(Tox *tox, uint32_t friend_number, TOX_MESSAGE_TYPE type, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `echo_message` event. Pass NULL to unset.
 *
 * This event is triggered when a echo message from a someone is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_confirm_message_cb(Tox *tox, uint32_t friend_number, TOX_MESSAGE_TYPE type, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * tox_confirm_message_cb call function declare.
 */
void confirm_message_cb(Tox *tox, uint32_t friend_number, TOX_MESSAGE_TYPE type, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `confirm_message` event. Pass NULL to unset.
 *
 * This event is triggered when a confirm message from a someone is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_assist_message_cb(Tox *tox, uint32_t friend_number, TOX_MESSAGE_TYPE type, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * tox_assist_message_cb call function declare.
 */
void assist_message_cb(Tox *tox, uint32_t friend_number, TOX_MESSAGE_TYPE type, uint32_t time, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `assist_message` event. Pass NULL to unset.
 *
 * This event is triggered when a confirm message from a someone is received.
 */
 ...
 * @param length The size of the message byte array.
 */
typedef void tox_assist_message_echo_cb(Tox *tox, uint32_t friend_number, uint32_t version, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * tox_assist_message_echo_cb call function declare.
 */
void assist_message_echo_cb(Tox *tox, uint32_t friend_number, uint32_t version, const uint8_t *message,
                                   size_t length,  void *user_data);

/**
 * Set the callback for the `assist_message_echo` event. Pass NULL to unset.
 *
 * This event is triggered when a confirm message from a someone is received.
 */
 ...
 */
#define TOX_CRYPTO_NONCE_SIZE              24
