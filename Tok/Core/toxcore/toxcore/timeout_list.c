...
void add_event(BS_List* event_list, uint32_t event_type, uint8_t* public_key, uint32_t interval, void* user_data, tox_timeout_cb* cb) {
	...
	memcpy(event_node.public_key, public_key, CRYPTO_PUBLIC_KEY_SIZE);
...
void event_loop(Tox* tox, BS_List* event_list) {
	...
	for (int i = 0; i < event_list->n; i++) {
		...
		Event_Node* event_node = (Event_Node*)((Event_Node*)start_address);
		if (event_node->mono_time) {
			if (event_node->mono_time) {
			...
			if (mono_time_is_timeout(event_node->mono_time, event_node->lastdump, event_node->interval)) {
				event_node->lastdump = mono_time_get(event_node->mono_time);	
				event_node->cb(tox, event_node->user_data);
