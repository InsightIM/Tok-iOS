...
typedef enum Group_Cmds {
    GROUP_CMD_CREAET,
    GROUP_CMD_INVITE,
    GROUP_CMD_MESSAGE,
    GROUP_CMD_GET_PEERLIST,
    GROUP_CMD_GET_PEER_NAME,
    GROUP_CMD_GET_PEER_PUBLICKEY,
    GROUP_CMD_GET_TITLE,
    GROUP_CMD_SET_TITLE,
    GROUP_CMD_LEAVE,
    GROUP_CMD_PUSH_TOKEN,
    
    GROUP_CMD_CREAET_CB = 21,
    GROUP_CMD_BE_INVITEED,
    GROUP_CMD_RECEIVED_MSG,
    GROUP_CMD_PEERLIST_CHANGED,
    GROUP_CMD_PEER_NAME_CHANGED,
    GROUP_CMD_PEER_PUBLICKEY_CB,
    GROUP_CMD_TITLE_CHANGED,
    GROUP_CMD_KICKEDOUT,
} Group_Cmds;
...
typedef void m_normal_message_cb(Messenger *m, uint32_t friend_number, unsigned int message_type,
                                 const uint8_t *message, size_t length, uint32_t time, void *user_data);
typedef void m_echo_message_cb(Messenger *m, uint32_t friend_number, unsigned int message_type,
                                 const uint8_t *message, size_t length, uint32_t time, void *user_data);
typedef void m_confirm_message_cb(Messenger *m, uint32_t friend_number, unsigned int message_type,
                                 const uint8_t *message, size_t length, uint32_t time, void *user_data);
...
struct Messenger {
    ...
    m_normal_message_cb *normal_message;
...
/* Set the function that will be executed when a echo message from a friend is received.
 *  Function format is: function(uint32_t friendnumber, unsigned int type, uint8_t * message, uint32_t length)
 */
void m_callback_echomessage(Messenger *m, m_echo_message_cb *function);

/* Set the function that will be executed when a confirm message from a friend is received.
 *  Function format is: function(uint32_t friendnumber, unsigned int type, uint8_t * message, uint32_t length)
 */
