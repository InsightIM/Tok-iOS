...
class WelcomeViewController: BaseViewController {
    ...
    lazy var registerButton: UIButton = {
        let button = UIButton()
        button.fcStyle(title: NSLocalizedString("Create Account", comment: ""), color: .white, titleColor: .tokBlue)
        return button
    }()
    ...
    lazy var importButton: UIButton = {
        let button = UIButton(type: .system)
        let attributedString = NSAttributedString(string: NSLocalizedString("Import your tox profile", comment: ""),
    ...
    override func viewDidLoad() {
        ...
        registerButton.rx
            ...
            .subscribe(onNext: { [weak self] _ in
                ...
                vc.title = NSLocalizedString("Create Account", comment: "")
                self?.navigationController?.pushViewController(vc, animated: true)
            })
            .disposed(by: disposeBag)
