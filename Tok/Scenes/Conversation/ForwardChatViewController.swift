...
class ForwardChatViewController: BaseViewController {
    ...
    init(viewModel: ForwardMessageViewModel) {
        ...
        chats = UserService.shared.toxMananger!.objects.chats().sortedResultsUsingProperty("lastActivityDateInterval", ascending: false)
...
extension ForwardChatViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            cell.avatarImageView.setImage(by: chat)
            if chat.isGroup {
