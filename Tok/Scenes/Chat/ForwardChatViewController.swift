...
class ForwardChatViewController: BaseViewController {
    ...
    init(viewModel: ForwardMessageViewModel) {
        ...
        chats = viewModel.messageService.database.normalChats().sortedResultsUsingProperty("lastActivityDateInterval", ascending: false).toList()
...
extension ForwardChatViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            cell.avatarImageView.image = AvatarManager.shared.chatAvatar(chatId: chat.uniqueIdentifier, database: viewModel.messageService.database)
            if chat.isGroup {
