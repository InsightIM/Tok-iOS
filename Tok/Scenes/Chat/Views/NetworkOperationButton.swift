...
class NetworkOperationButton: UIButton {
    ...
    func update(with newStyle: Style, oldStyle: Style) {
        ...
        case .finished(let showPlayIcon):
            ...
            if showPlayIcon {
                setImage(UIImage(named: "ic_play"), for: .normal)
            } else {
        ...
        case .upload:
            ...
            setImage(UIImage(named: "ic_file_upload"), for: .normal)
            isUserInteractionEnabled = true
        case .download:
            backgroundView.isHidden = false
            setImage(UIImage(named: "ic_file_download"), for: .normal)
            isUserInteractionEnabled = true
        case .expired:
            backgroundView.isHidden = false
            setImage(UIImage(named: "ic_file_expired"), for: .normal)
            isUserInteractionEnabled = false
        case .busy(let progress):
            backgroundView.isHidden = false
            if !oldStyle.isBusy {
                setImage(UIImage(named: "ic_file_cancel"), for: .normal)
            }
