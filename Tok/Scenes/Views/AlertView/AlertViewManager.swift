...
class AlertViewManager {
    ...
    static func showMessageSheet(with title: String, cancelTitle: String?) {
    ...
    static func showMessageSheet(with title: String, actions: [(String, UIAlertAction.Style, Action?)]) {
        let view: AlertMessageView = try! SwiftMessages.viewFromNib()
        view.bounceAnimationOffset = 0
        var config = SwiftMessages.Config()
        config.presentationStyle = .bottom
