...
class ChooseFriendViewController: BaseViewController {
    ...
    init(messageService: MessageService) {
        ...
        let realmFriends = messageService.database.normalFriends()
        friends = realmFriends.toList().map { FriendSelectionViewModel(friend: $0, messageService: messageService) }
...
extension ChooseFriendViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        guard viewModel.isSelected.value == false else {
