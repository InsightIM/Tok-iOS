...
fileprivate extension AppCoordinator {
    ...
    func reactiveFlow() {
        ...
        UserService.shared.tryLogin()
            .flatMapLatest { [unowned self] managers -> Observable<Void> in
                let isAutorized = managers != nil
                ...
                case .home:
                    runningFlow = self.runMainFlow(tokManager: managers!.1)
    ...
    func runMainFlow(tokManager: TokManager) -> Observable<Void> {
        let homeCoordinator = HomeCoordinator(window: window, tokManager: tokManager)
